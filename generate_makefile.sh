...
echo "libinspur_dbus_cpp_SOURCES = \\"
for i in ${interfaces};
do
...
echo "libinspur_dbus_hpp_SOURCES = \\"
for i in ${interfaces};
do
...
cat << MAKEFILE
libinspur_dbus.cpp: \$(libinspur_dbus_cpp_SOURCES)
	cat \$^ > \$@

nobase_include_HEADERS = \$(libinspur_dbus_hpp_SOURCES)

.PHONY: clean-dbus
clean-dbus:
	for i in \$(libinspur_dbus_cpp_SOURCES) \\
	         \$(libinspur_dbus_hpp_SOURCES); \\
	do \\
	    test -e \$\$i && rm \$\$i ; \\
